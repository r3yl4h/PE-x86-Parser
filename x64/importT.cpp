            ...
            if (importTable->VirtualAddress >= sectionHeader[i].VirtualAddress && importTable->VirtualAddress < sectionHeader[i].VirtualAddress + sectionHeader[i].SizeOfRawData) {
                ...
                while (importDesc->Name != 0) {
                    ...
                    while (thunkData->u1.AddressOfData != 0) {
                        ...
                        std::cout << "\tAddress: 0x" << std::hex << thunkData->u1.Function + optHeader64->ImageBase
                                  << "\tDLL: " << std::setw(20) << std::left << dllName
                                  << " \tFunction: " << std::setw(20) << std::left << (name_func == nullptr ? "Ordinal" : name_func) << std::setw(0) << std::endl;
                        ++thunkData;
                    }
                    ++importDesc;
                }
