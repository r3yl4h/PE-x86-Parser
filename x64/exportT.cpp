...
void getExport64(std::ifstream &file) {
    if (!file.is_open()) {
        ...
        for (int i = 0; i < ntHeaders->FileHeader.NumberOfSections; ++i) {
            if (exportTable->VirtualAddress >= sectionHeader[i].VirtualAddress &&
                ...
                for (DWORD j = 0; j < exportDir->NumberOfNames; ++j) {
                    ...
                    std::cout << "\tAddress: 0x" << std::hex << addr_rva + optHeader64->ImageBase << std::dec << std::setw(20)
                              << "\tOrdinal: " << addrOrd[j] + exportDir->Base << std::setw(20)
                              << "\tFunction: " << ((funcN ? funcN : "Ordinal")) << std::setw(0) << std::endl;
                }
                break;
            }
        }
