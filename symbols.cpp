...
void get_symbole(const char* path) {
    ...
    if (SymGetModuleInfo(hProc, baseAddr, &modInfo)) {
        ...
        if (!SymEnumSymbols(hProc, baseAddr, nullptr, [](PSYMBOL_INFO pInfo, ULONG Size, PVOID Context) -> BOOL {
            ...
            std::cout << "       Value: " << pInfo->Value << std::endl;
            std::cout << "        Tag: " << GetTagString(pInfo->Tag) << std::endl;
            std::cout << "         Reserved: " << pInfo->Reserved << std::endl;
            std::cout << "        Scope: 0x" << pInfo->Scope << std::endl;
            std::cout << "       NameLen: 0x" << pInfo->NameLen << std::endl;
            std::cout << "      Flags: " << pInfo->Flags << std::endl;
            std::cout << "     Tag: 0x" << pInfo->Tag << std::endl;
            std::cout << std::endl;
            return TRUE;
